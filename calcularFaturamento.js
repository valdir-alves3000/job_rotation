function calcularFaturamento(vetorFaturamento) {
  const vetorFaturamentoLength = vetorFaturamento.length;
  let numeroDias = 0;
  let somaFaturamento = 0;
  let maiorValor = Number.MIN_SAFE_INTEGER;
  let menorValor = Number.MAX_SAFE_INTEGER;
  let numeroDiasAcimadaMedia = 0;
  let mediaMensal = 0;

  for (let i = 0; i < vetorFaturamentoLength; i++) {
    const faturamento = vetorFaturamento[i].valor;
    if (faturamento > 0) {
      numeroDias++;
  ...
  mediaMensal = somaFaturamento / numeroDias;
...
console.log(calcularFaturamento(data));
